...
public class GATKRealignmentNodeDialog extends HTExecutorNodeDialog {
	 ...
	 * known indels: 1000G phase 1 indels boolean use Mills and 1000G gold
	 * standard indels set String path to known indels: Mills and 1000G gold
	 * standards indels boolean use interval file String path to interval file
	 * in bed format, perform only realignment in that interval int number of
	 * threads
	 */

	final SettingsModelString gatk = new SettingsModelString(
			GATKRealignmentNodeModel.CFGKEY_GATK, "");
	final SettingsModelString reffile = new SettingsModelString(
			GATKRealignmentNodeModel.CFGKEY_REF_GENOME, "");
	final SettingsModelBoolean use_phase1_1000G = new SettingsModelBoolean(
			GATKRealignmentNodeModel.CFGKEY_USE_PHASE1_1000G,
			GATKRealignmentNodeModel.DEF_USE_PHASE1_1000G);
	final SettingsModelString phase1_1000G_file = new SettingsModelString(
			GATKRealignmentNodeModel.CFGKEY_PHASE1_1000G_FILE,
			GATKRealignmentNodeModel.DEF_PHASE1_1000G_FILE);
	final SettingsModelBoolean use_mills_1000G = new SettingsModelBoolean(
			GATKRealignmentNodeModel.CFGKEY_USE_MILLS_1000G,
			GATKRealignmentNodeModel.DEF_USE_MILLS_1000G);
	final SettingsModelString mills_1000G_file = new SettingsModelString(
			GATKRealignmentNodeModel.CFGKEY_MILLS_1000G_FILE,
			GATKRealignmentNodeModel.DEF_MILLS_1000G_FILE);
	final SettingsModelBoolean use_interval = new SettingsModelBoolean(
			GATKRealignmentNodeModel.CFGKEY_USE_INTERVAL,
			GATKRealignmentNodeModel.DEF_USE_INTERVAL);
	final SettingsModelString interval_file = new SettingsModelString(
			GATKRealignmentNodeModel.CFGKEY_INTERVAL_FILE,
			GATKRealignmentNodeModel.DEF_INTERVAL_FILE);
	final SettingsModelIntegerBounded num_threads = new SettingsModelIntegerBounded(
			GATKRealignmentNodeModel.CFGKEY_NUM_THREADS,
			GATKRealignmentNodeModel.DEF_NUM_THREADS,
			GATKRealignmentNodeModel.MIN_NUM_THREADS,
			GATKRealignmentNodeModel.MAX_NUM_THREADS);
	final SettingsModelIntegerBounded memory_usage = new SettingsModelIntegerBounded(
			GATKRealignmentNodeModel.CFGKEY_JAVAMEMORY,
			GATKRealignmentNodeModel.DEF_NUM_JAVAMEMORY,
			GATKRealignmentNodeModel.MIN_NUM_JAVAMEMORY,
			GATKRealignmentNodeModel.MAX_NUM_JAVAMEMORY);

	// tab TargetCreator
	/*
	 * int max interval size for longer intervals no realignment will be
	 * performed because it is too time consuming (500) int min reads at locus
	 * to do entropy calculation (4) double mismatch fraction of base misamtches
	 * to have high entropy, only needed if alignment was performed by an
	 * ungapped aligner (0.0, 0<...<1) int window size, two SNPs or regions of
	 * high entropy within the window are clusterd together (10) (>1)
	 */

	final SettingsModelIntegerBounded max_interval = new SettingsModelIntegerBounded(
			GATKRealignmentNodeModel.CFGKEY_MAX_INTERVAL,
			GATKRealignmentNodeModel.DEF_MAX_INTERVAL,
			GATKRealignmentNodeModel.MIN_MAX_INTERVAL,
			GATKRealignmentNodeModel.MAX_MAX_INTERVAL);
	final SettingsModelIntegerBounded min_reads = new SettingsModelIntegerBounded(
			GATKRealignmentNodeModel.CFGKEY_MIN_READS,
			GATKRealignmentNodeModel.DEF_MIN_READS,
			GATKRealignmentNodeModel.MIN_MIN_READS,
			GATKRealignmentNodeModel.MAX_MIN_READS);
	final SettingsModelIntegerBounded window = new SettingsModelIntegerBounded(
			GATKRealignmentNodeModel.CFGKEY_WINDOW,
			GATKRealignmentNodeModel.DEF_WINDOW,
			GATKRealignmentNodeModel.MIN_WINDOW,
			GATKRealignmentNodeModel.MAX_WINDOW);
	final SettingsModelDoubleBounded mismatch = new SettingsModelDoubleBounded(
			GATKRealignmentNodeModel.CFGKEY_MISMATCH,
			GATKRealignmentNodeModel.DEF_MISMATCH,
			GATKRealignmentNodeModel.MIN_MISMATCH,
			GATKRealignmentNodeModel.MAX_MISMATCH);

	// tab IndelRealigner
	/*
	 * String consensus determination model, how to compute consensus ->
	 * USE_READS, KNOWNS_ONLY, USE_SW (USE_READS) double LOD threshold for
	 * cleaning, significance threshold of position for realignment (5.0)
	 * 
	 * advanced parameters -> reduce time for realignment for high coverage data
	 * double entropy threshold, percentage of mismatches at a genomic position,
	 * defines high entropy (0.15) int maximal number of consensuses to try (30)
	 * int max insert size for realignment of paired-end reads (3000) int
	 * maximum number of base shifts allow when realigning a read (200) int max
	 * number of reads used for determining for consensuses (120) int max reads
	 * for realignemnt, if there are more reads no realignment will be performed
	 * (20000)
	 * 
	...
	final SettingsModelString consensus_model = new SettingsModelString(
			GATKRealignmentNodeModel.CFGKEY_CONSENSUS_MODEL,
			GATKRealignmentNodeModel.DEF_CONSENSUS_MODEL);
	final SettingsModelDoubleBounded lod_threshold = new SettingsModelDoubleBounded(
			GATKRealignmentNodeModel.CFGKEY_LOD_THRESHOLD,
			GATKRealignmentNodeModel.DEF_LOD_THRESHOLD,
			GATKRealignmentNodeModel.MIN_LOD_THRESHOLD,
			GATKRealignmentNodeModel.MAX_LOD_THRESHOLD);
	final SettingsModelDoubleBounded entropy = new SettingsModelDoubleBounded(
			GATKRealignmentNodeModel.CFGKEY_ENTROPY,
			GATKRealignmentNodeModel.DEF_ENTROPY,
			GATKRealignmentNodeModel.MIN_ENTROPY,
			GATKRealignmentNodeModel.MAX_ENTROPY);
	final SettingsModelIntegerBounded max_consensuses = new SettingsModelIntegerBounded(
			GATKRealignmentNodeModel.CFGKEY_MAX_CONSENSUSES,
			GATKRealignmentNodeModel.DEF_MAX_CONSENSUSES,
			GATKRealignmentNodeModel.MIN_MAX_CONSENSUSES,
			GATKRealignmentNodeModel.MAX_MAX_CONSENSUSES);
	final SettingsModelIntegerBounded max_isize = new SettingsModelIntegerBounded(
			GATKRealignmentNodeModel.CFGKEY_MAX_ISIZE,
			GATKRealignmentNodeModel.DEF_MAX_ISIZE,
			GATKRealignmentNodeModel.MIN_MAX_ISIZE,
			GATKRealignmentNodeModel.MAX_MAX_ISIZE);
	final SettingsModelIntegerBounded max_pos_move = new SettingsModelIntegerBounded(
			GATKRealignmentNodeModel.CFGKEY_MAX_POS_MOVE,
			GATKRealignmentNodeModel.DEF_MAX_POS_MOVE,
			GATKRealignmentNodeModel.MIN_MAX_POS_MOVE,
			GATKRealignmentNodeModel.MAX_MAX_POS_MOVE);
	final SettingsModelIntegerBounded max_reads_cons = new SettingsModelIntegerBounded(
			GATKRealignmentNodeModel.CFGKEY_MAX_READS_CONS,
			GATKRealignmentNodeModel.DEF_MAX_READS_CONS,
			GATKRealignmentNodeModel.MIN_MAX_READS_CONS,
			GATKRealignmentNodeModel.MAX_MAX_READS_CONS);
	final SettingsModelIntegerBounded max_reads_realign = new SettingsModelIntegerBounded(
			GATKRealignmentNodeModel.CFGKEY_MAX_READS_REALIGN,
			GATKRealignmentNodeModel.DEF_MAX_READS_REALIGN,
			GATKRealignmentNodeModel.MIN_MAX_READS_REALIGN,
			GATKRealignmentNodeModel.MAX_MAX_READS_REALIGN);
	...
	private void createGeneralOptions() {
		...
		addDialogComponent(new DialogComponentBoolean(use_phase1_1000G,
				"Use 1000 genomes phase1 indel set (improves quality of realignment)"));
		DialogComponentFileChooser p1f = new DialogComponentFileChooser(
				phase1_1000G_file, "p11000g", JFileChooser.OPEN_DIALOG, false,
				".vcf");
		...
		use_phase1_1000G.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				if (!phase1) {
					if (!use_phase1_1000G.getBooleanValue()
							&& !phase1_1000G_file.isEnabled()) {
						phase1 = true;
					} else {
						phase1_1000G_file.setEnabled(use_phase1_1000G
								.getBooleanValue());
					}
				}
		...
		addDialogComponent(new DialogComponentBoolean(
				use_mills_1000G,
				"Use Mills and 1000 genomes gold standard indel set (improves quality of realignment)"));
		DialogComponentFileChooser millsf = new DialogComponentFileChooser(
				mills_1000G_file, "m1000ggs", JFileChooser.OPEN_DIALOG, false,
				".vcf");
		...
		use_mills_1000G.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				if (!mills) {
					if (!use_mills_1000G.getBooleanValue()
							&& !mills_1000G_file.isEnabled()) {
						mills = true;
					} else {
						mills_1000G_file.setEnabled(use_mills_1000G
								.getBooleanValue());
					}
				}
		...
		addDialogComponent(new DialogComponentBoolean(use_interval,
				"Restrict realignment to certain genomic regions"));
		interval_file.setEnabled(false);
		addDialogComponent(new DialogComponentFileChooser(interval_file,
				"ifile", JFileChooser.OPEN_DIALOG, false, ".bed", ".intervals"));
	...
	private void createTargetCreatorOptions() {
		...
		addDialogComponent(new DialogComponentNumber(max_interval,
				"Maximum interval length", 1, 5));

		createNewGroup("Minimum number of reads for entropy calculation");
		addDialogComponent(new DialogComponentNumber(min_reads,
				"Minimum number of reads", 1, 5));

		createNewGroup("Fraction of mismatching base qualities (only for ungapped aligner)");
		addDialogComponent(new DialogComponentNumber(
				mismatch,
				"Fraction of base quality mismatches at a position to have high entropy",
				0.01, 5));
	...
	private void createIndelRealignmentOptions() {
		...
		addDialogComponent(new DialogComponentStringSelection(consensus_model,
		...
		addDialogComponent(new DialogComponentNumber(lod_threshold,
				"LOD Threshold", 0.1, 5));

		createNewGroup("Entropy threshold");
		addDialogComponent(new DialogComponentNumber(entropy,
				"Entropy threshold", 0.01, 5));

		createNewGroup("Maximum number of consensus sequences to try");
		addDialogComponent(new DialogComponentNumber(max_consensuses,
				"Consensus threshold", 1, 5));

		createNewGroup("Maximum insert size for realignment of paired-end reads");
		addDialogComponent(new DialogComponentNumber(max_isize,
				"Insert size threshold", 10, 5));

		createNewGroup("Maximum distance of read to original alignment position");
		addDialogComponent(new DialogComponentNumber(max_pos_move,
				"Read shift threshold", 1, 5));

		createNewGroup("Maximum number of reads used for consensus calculation");
		addDialogComponent(new DialogComponentNumber(max_reads_cons,
				"Reads for consensus threshold", 1, 5));

		createNewGroup("Maximum number of reads for realignment");
		addDialogComponent(new DialogComponentNumber(max_reads_realign,
				"Reads for realignemnt threshold", 100, 5));

		createNewGroup("Output");
		addDialogComponent(new DialogComponentBoolean(alignment_tag,
				"Do not output original cigar string"));

		createNewGroup("Further options");
		addDialogComponent(new DialogComponentOptionalString(m_opt_flags,
				"Further flags"));
