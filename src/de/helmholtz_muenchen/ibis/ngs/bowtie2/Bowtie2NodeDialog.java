 ...
 * @author 
 */
public class Bowtie2NodeDialog extends DefaultNodeSettingsPane {
    ...
    protected Bowtie2NodeDialog() {
    	...
    	final SettingsModelString refseq = new SettingsModelString(Bowtie2NodeModel.CFGKEY_REFSEQFILE,null);
    	final SettingsModelString readType = new SettingsModelString(Bowtie2NodeModel.CFGKEY_READTYPE,"single-end");
    	...
    	final SettingsModelBoolean usecutoff = new SettingsModelBoolean(Bowtie2NodeModel.CFGKEY_USECUTOFF, false);
    	final SettingsModelIntegerBounded cutoff = new SettingsModelIntegerBounded(Bowtie2NodeModel.CFGKEY_CUTOFF, Bowtie2NodeModel.DEFAULT_CUTOFF, 1, Integer.MAX_VALUE);
    	...
    	final SettingsModelBoolean nodiscordant = new SettingsModelBoolean(Bowtie2NodeModel.CFGKEY_NODISCORDAND, false);
    	...
    	createNewGroup("Bowtie2 installpath.");
    	addDialogComponent(new DialogComponentFileChooser(new SettingsModelString(Bowtie2NodeModel.CFGKEY_INSTALLPATH,null), "his_bow_id", 0, true));
    	createNewGroup("Reference sequence: FastA file (e.g. genome)");
    	addDialogComponent(new DialogComponentFileChooser(refseq, "his1_bow_id", 0, ""));
    	createNewGroup("Read types");
    	...
    	addDialogComponent(new DialogComponentBoolean(new SettingsModelBoolean(Bowtie2NodeModel.CFGKEY_NODC, false), "Disable use of the difference-cover sample (algorithm becomes quadratic)"));
    	...
    	addDialogComponent(new DialogComponentBoolean(usecutoff, "Index only the first <int> bases of the reference sequences and ignore the rest:"));
    	addDialogComponent(new DialogComponentNumber(cutoff,"", /*step*/ 1));
    	...
    	setHorizontalPlacement(false);
    	addDialogComponent(new DialogComponentNumber(new SettingsModelIntegerBounded(Bowtie2NodeModel.CFGKEY_TRIM5, Bowtie2NodeModel.DEFAULT_TRIM5, 0, Integer.MAX_VALUE),
    			"Trim <int> bases from 5'/left end of reads:", /*step*/ 1));
    	addDialogComponent(new DialogComponentNumber(new SettingsModelIntegerBounded(Bowtie2NodeModel.CFGKEY_TRIM3, Bowtie2NodeModel.DEFAULT_TRIM3, 0, Integer.MAX_VALUE),
    			"Trim <int> bases from 3'/right end of reads:", /*step*/ 1));
    	addDialogComponent(new DialogComponentStringSelection(new SettingsModelString(Bowtie2NodeModel.CFGKEY_QUALS,""),"Select quality score type:", 
    	...
    	setHorizontalPlacement(false);
    	addDialogComponent(new DialogComponentNumber(new SettingsModelIntegerBounded(Bowtie2NodeModel.CFGKEY_DPAD, Bowtie2NodeModel.DEFAULT_DPAD, 0, Integer.MAX_VALUE),
    			"Include <int> extra ref chars on sides of DP table:", /*step*/ 1));
    	addDialogComponent(new DialogComponentNumber(new SettingsModelIntegerBounded(Bowtie2NodeModel.CFGKEY_GBAR, Bowtie2NodeModel.DEFAULT_GBAR, 0, Integer.MAX_VALUE),
    			"Disallow gaps within <int> nucs of read extremes:", /*step*/ 1));
    	addDialogComponent(new DialogComponentBoolean(new SettingsModelBoolean(Bowtie2NodeModel.CFGKEY_IGNOREQUALS, false), "Treat all quality values as 30 on Phred scale."));
    	addDialogComponent(new DialogComponentBoolean(new SettingsModelBoolean(Bowtie2NodeModel.CFGKEY_NOFW, false), "Do not align forward (original) version of read."));
    	addDialogComponent(new DialogComponentBoolean(new SettingsModelBoolean(Bowtie2NodeModel.CFGKEY_NORC, false), "Do not align reverse-complement version of read."));
    	...
    	addDialogComponent(new DialogComponentNumber(new SettingsModelIntegerBounded(Bowtie2NodeModel.CFGKEY_MP, Bowtie2NodeModel.DEFAULT_MP, 0, Integer.MAX_VALUE),
    			"Max penalty for mismatch (lower qual = lower penalty):", /*step*/ 1));
    	addDialogComponent(new DialogComponentNumber(new SettingsModelIntegerBounded(Bowtie2NodeModel.CFGKEY_NP, Bowtie2NodeModel.DEFAULT_NP, 0, Integer.MAX_VALUE),
    			"Penalty for non-A/C/G/Ts in read/ref:", /*step*/ 1));
    	setHorizontalPlacement(true);
    	addDialogComponent(new DialogComponentNumber(new SettingsModelIntegerBounded(Bowtie2NodeModel.CFGKEY_RDG1, Bowtie2NodeModel.DEFAULT_RDG1, 0, Integer.MAX_VALUE),
    			"Read gap open penalty:", /*step*/ 1));
    	addDialogComponent(new DialogComponentNumber(new SettingsModelIntegerBounded(Bowtie2NodeModel.CFGKEY_RDG2, Bowtie2NodeModel.DEFAULT_RDG2, 0, Integer.MAX_VALUE),
    			"Read gap extend penalty:", /*step*/ 1));
    	setHorizontalPlacement(false);
    	setHorizontalPlacement(true);
    	addDialogComponent(new DialogComponentNumber(new SettingsModelIntegerBounded(Bowtie2NodeModel.CFGKEY_RFG1, Bowtie2NodeModel.DEFAULT_RFG1, 0, Integer.MAX_VALUE),
    			"Reference gap open penalty:", /*step*/ 1));
    	addDialogComponent(new DialogComponentNumber(new SettingsModelIntegerBounded(Bowtie2NodeModel.CFGKEY_RFG2, Bowtie2NodeModel.DEFAULT_RFG2, 0, Integer.MAX_VALUE),
    	...
    	usecutoff.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				cutoff.setEnabled(usecutoff.getBooleanValue());
			}
		});
