...
public class SamToolsNodeDialog extends HTExecutorNodeDialog {
	...
	public void addToolDialogComponents() {
		...
		// final SettingsModelBoolean doCapMapQual = new
		// SettingsModelBoolean(SamToolsNodeModel.CFGKEY_DOCAPMAPQUAL, false);
		// final SettingsModelIntegerBounded capMapQual = new
		// SettingsModelIntegerBounded(SamToolsNodeModel.CFGKEY_CAPMAPQUAL, 1,
		// 1, Integer.MAX_VALUE);
		// rmdup
		...
		// cat
		final SettingsModelBoolean useHeaderSAM = new SettingsModelBoolean(SamToolsNodeModel.CFGKEY_USEHEADERSAM,
				false);
		...
		// final SettingsModelString inBAM2 = new
		// SettingsModelString(SamToolsNodeModel.CFGKEY_INBAM2, "");
		// reheader
		// final SettingsModelString rehInBAM = new
		// SettingsModelString(SamToolsNodeModel.CFGKEY_REHINBAM, "");
		...
		// SettingsModelString minbam2 = new
		// SettingsModelString(SamToolsNodeModel.CFGKEY_MINBAM2, "");
		final SettingsModelBoolean mcompression = new SettingsModelBoolean(SamToolsNodeModel.CFGKEY_MCOMPRESSION,
				false);
		...
		final SettingsModelBoolean muncompressed = new SettingsModelBoolean(SamToolsNodeModel.CFGKEY_MUNCOMPRESSED,
				false);
		// faidx
		// final SettingsModelString infasta = new
		// SettingsModelString(SamToolsNodeModel.CFGKEY_INFASTA, "");
		// phase
		final SettingsModelIntegerBounded blocklength = new SettingsModelIntegerBounded(
				SamToolsNodeModel.CFGKEY_BLOCKLENGTH, 13, 1, Integer.MAX_VALUE);
		final SettingsModelString prefix = new SettingsModelString(SamToolsNodeModel.CFGKEY_PREFIX, "phase");
		final SettingsModelIntegerBounded hetphred = new SettingsModelIntegerBounded(SamToolsNodeModel.CFGKEY_HETPHRED,
				37, 1, Integer.MAX_VALUE);
		final SettingsModelIntegerBounded minqual = new SettingsModelIntegerBounded(SamToolsNodeModel.CFGKEY_MINQUAL,
				13, 1, Integer.MAX_VALUE);
		final SettingsModelIntegerBounded maxdepth = new SettingsModelIntegerBounded(SamToolsNodeModel.CFGKEY_MAXDEPTH,
		...
		// final SettingsModelBoolean dropambig = new
		// SettingsModelBoolean(SamToolsNodeModel.CFGKEY_DROPAMBIG, false);
		...
		addDialogComponent(new DialogComponentStringSelection(utility, "Select Utility", "cat", "faidx", "calmd",
				"fixmate", "flagstat", "idxstats", "merge", "phase", "reheader", "rmdup"));

		// createNewGroup("SamTools");
		// addDialogComponent(new DialogComponentFileChooser(samtools,
		// "SamTools", 0,""));
		...
		// addDialogComponent(new DialogComponentFileChooser(refseqfile,
		// "Reference sequence file", 0,""));
		...
		// addDialogComponent(new DialogComponentFileChooser(inBAM2, "Second BAM
		// file", 0,".bam", ".BAM"));

		// createNewTab("faidx");
		// createNewGroup("Fasta file to index:");
		// addDialogComponent(new DialogComponentFileChooser(infasta, "", 0,
		// false));
		...
		addDialogComponent(new DialogComponentStringSelection(compression, "", "compressed", "uncompressed"));// use
																												// uncompressed
																												// for
																												// pipelining!
		setHorizontalPlacement(false);
		...
		/*
		 * setHorizontalPlacement(true); addDialogComponent(new
		 * DialogComponentBoolean(doCapMapQual,
		 * "Use coefficient to cap mapping quality of poorly mapped reads"));
		...
		// addDialogComponent(new DialogComponentFileChooser(minbam2, "Second
		// BAM file", 0,".bam", ".BAM"));
		...
		utility.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				...
				// fixmate
				if (utility.getStringValue().equals("fixmate") || utility.getStringValue().equals("flagstat")
						|| utility.getStringValue().equals("idxstats") || utility.getStringValue().equals("rmdup")) {
		...
		/*
		 * doCapMapQual.addChangeListener(new ChangeListener() { public void
		 * stateChanged(ChangeEvent e) { if(doCapMapQual.getBooleanValue()){
		 * capMapQual.setEnabled(true); } else{ capMapQual.setEnabled(false); }
		 * } });
		 */
	...
	// String toolPath =
	// IBISKNIMENodesPlugin.getDefault().getToolPathPreference("samtools");
	...
	//
	// String refGenome =
	// IBISKNIMENodesPlugin.getDefault().getRefGenomePreference();
