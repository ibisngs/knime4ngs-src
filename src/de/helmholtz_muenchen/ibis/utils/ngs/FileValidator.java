...
public class FileValidator {
	 ...
	 */
	public static int versionControl(String path2tools,String toolname) throws IOException, InterruptedException{
			...
			if (line) {
				...
				if (VERSION.equals(curr_version)) {
					...
					if (toolname.equals("BWA")) {
						...
						char last = curr_version
								.charAt(curr_version.length() - 1);
						boolean isAnInt = last >= '0' && last <= '9';
						if (!isAnInt) {
							curr_version = curr_version.substring(0,
									curr_version.length() - 1);
						}
						if (VERSION.equals(curr_version)) {
							return 0;
						}
